...
# Đọc ảnh đầu vào
input_image = cv2.imread('Assets/Image/Picture1.png', cv2.IMREAD_GRAYSCALE)
...
# Bước 4: Tạo bộ lọc thông thấp Ideal
D0 = 5  # Thay đổi D0 theo ý muốn
u = np.arange(P)
v = np.arange(Q)
U, V = np.meshgrid(u, v)
U = U - P/2
...
# Bước 5: Áp dụng bộ lọc thông thấp
G = np.multiply(F, H.T)  # Chuyển vị của H bằng H.T
